...
def generate_xlsx_file_for_consolidation_groups():
	...
	head_row = ["S/No", "Subject of procurement", "Currency", "Estimated cost", "Source of funding", "Procurement method",
	"Contract type", "PRE-QUALIFICATION (Yes or No)", "Bid invitation date", "Bid closing/opening date",
	"Approval of evaluation date", "Award notication date", "Contract signing date", 
	...
	for group in groups:
		if group.plan_set.count():
			columns = [count, group.subject_of_procurement, "UGX", group.estimated_cost(), "GOU", group.method_of_procurement, 
